...
test("chính sách mật khẩu chặn mật khẩu ngắn và phổ biến", () => {
  ...
  assert.throws(() => assertPasswordPolicy("taopho88"), /quá phổ biến/);
  assert.throws(() => assertPasswordPolicy("x".repeat(201)), /tối đa 200/);
  // Mật khẩu hợp lệ thì không ném.
  assertPasswordPolicy("taophohoanhai2026");
});

test("băm mật khẩu dùng salt riêng cho từng lần và kiểm lại đúng", async () => {
  const first = await hashPassword("taophohoanhai2026");
  const second = await hashPassword("taophohoanhai2026");
  ...
  assert.doesNotMatch(first.hash, /taopho/i, "hash không được chứa mật khẩu gốc");

  assert.equal(await verifyPassword("taophohoanhai2026", first), true);
  assert.equal(await verifyPassword("taophohoanhai2026", second), true);
  assert.equal(await verifyPassword("taophohoanhai2027", first), false);
...
test("số vòng lặp lưu theo bản ghi nên hash cũ vẫn kiểm được sau khi nâng cấu hình", async () => {
  ...
  const legacy = await hashPassword("taophohoanhai2026");
  const downgraded = { ...legacy, iterations: legacy.iterations };
  assert.equal(await verifyPassword("taophohoanhai2026", downgraded), true);
  ...
  assert.equal(await verifyPassword("taophohoanhai2026", { ...legacy, iterations: 1000 }), false);
});
